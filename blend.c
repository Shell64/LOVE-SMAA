...
#define SMAA_RT_METRICS vec4(1.0 / love_ScreenSize.x, 1.0 / love_ScreenSize.y, love_ScreenSize.x, love_ScreenSize.y)
...
varying vec2 pixcoord;
	...
	vec4 position(mat4 transform_projection, vec4 vertex_position)
	{
		...
		pixcoord = VertexTexCoord2.xy / SMAA_PIXEL_SIZE;
		SMAABlendingWeightCalculationVS(VertexTexCoord2.xy, pixcoord, offset);
...
vec4 effect( vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	return SMAABlendingWeightCalculationPS(UV, pixcoord, offset, texture, area_tex, search_tex, ivec4(0.0, 0.0, 0.0, 0.0));
}
#endif